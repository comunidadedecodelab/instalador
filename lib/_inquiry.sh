...
get_link_git() {
  ...
  print_banner
  printf "${WHITE} 💻 Insira o link do GITHUB do seu Whaticket Decode que deseja instalar:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git
}
...
inquiry_options() {
  ...
  print_banner
  printf "${WHITE} 💻 Bem vindo(a) ao Assistente Decode Lab, selecione abaixo a proxima ação!${GRAY_LIGHT}"
  printf "\n\n"
  printf "   [0] Instalar Whaticket Decode\n"
  printf "   [1] Atualizar Whaticket DecodeP\n"
  printf "   [2] Deletar Whaticket Decode\n"
  printf "   [3] Bloquear Whaticket DecodeP\n"
  printf "   [4] Desbloquear Whaticket Decode\n"
  printf "   [5] Alter. dominio Whaticket Decode\n"
  printf "\n"
  read -p "> " option
